...
for regularization in regularizations:
	...
	for label_ch_prob in label_ch_prob_all:
		...
		d_all = np.array([ 20, 50, 100, 150, 200, 500, 1000, 1500, 2000, 2500, 3000 , 5000 , 6000, 7000, 8000, 9000 ])
		#d_all = np.array([ 8000, 8200, 8400, 8600, 8800, 9000, 9200, 9400, 9600, 9800, 10000, 10200, 10400, 10600, 10800, 11000, 11200, 11400, 11600,\
		#					11800, 12000]) ## zoomed #RELU
		#d_all = np.array([ 300 , 500 ])
		# convert class vectors to binary class matrices
		y_train = keras.utils.to_categorical(y_train, num_classes)
		y_test = keras.utils.to_categorical(y_test, num_classes)
		
		#print ("x train orig shape: " + str(x_train.shape))
		#print ("x test orig shape: " + str(x_test.shape))
		...
		for d in d_all:
			...
			print  ("Z-relu-train is : " + str(x_train_rff_represent.shape) )

			x_test_rff_represent =  represe_rff.predict(x_test)  ### the feature matrix in RFF domain; this is ( #data_points * #RFF )
			print  ("Z-relu-test is : " + str(x_test_rff_represent.shape) )
	...
	pickle_name = 'RELU'  + '_mnist_' + str(num_training_data) +'_reg_'+str(regularization) + '_primal'  + '.p'
	...
	savepkl( '../pickle/mnist/relu/primal/'+ pickle_name , dict_final ) ## i am here
